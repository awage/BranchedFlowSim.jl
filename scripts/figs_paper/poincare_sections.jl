...
function plot_curves(V; a = 1, v0 = 1., dt = 0.01, T = 100000, x0 = 0, prefix = "poincare")
    ...
    ax = Axis(fig[1,1], ylabel = L"p_y", xlabel = L"y", yticklabelsize = 40, xticklabelsize = 40, ylabelsize = 40, xlabelsize = 40,  titlesize = 40)
    s2 = savename(string(prefix, "flow"),  @dict(a,v0,dt),  "png")
    fig2 = Figure(resolution = (800, 600))
    ax2 = Axis(fig2[1,1], ylabel = L"y", xlabel = L"t", yticklabelsize = 40, xticklabelsize = 40, ylabelsize = 40, xlabelsize = 40,  titlesize = 40)

    for (j,y) in enumerate(yrange)
        u,t = trajectory(df, T, [x0, y, py])
        λ = lyapunov(df, 10000; u0 = [x0, y, py])
        ...
        col =  λ > 1e-3 ? :red : :black
        scatter!(ax, x_ind, u[ind,3], markersize = 1.7, color = col, rasterize = 1)
        lines!(ax2, u[:,1], u[:,2], color = col)
    end
    save(plotsdir(s), fig)
    save(plotsdir(s2), fig2)
end
